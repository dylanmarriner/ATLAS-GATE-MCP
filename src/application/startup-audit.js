...
export const STARTUP_INVARIANTS = {
  ...
  SYSTEM_ERROR_CODES_COMPLETE: {
    id: 'INV_STARTUP_SYSTEM_ERROR_CODES_COMPLETE',
    category: 'Error Boundary',
...
export async function runStartupAudit(serverInstance, role) {
  ...
  try {
    ...
    const diagnostic = err instanceof SystemError ? err.toDiagnostic() : {
      error: err.message,
      stack: err.stack
    };
...
function checkErrorBoundary(result) {
  ...
  const codesInvariant = STARTUP_INVARIANTS.SYSTEM_ERROR_CODES_COMPLETE.id;

  try {
    if (!SYSTEM_ERROR_CODES || typeof SYSTEM_ERROR_CODES !== 'object') {
      throw new Error('SYSTEM_ERROR_CODES is not defined or not an object');
    }
    ...
    const missingCodes = requiredCodes.filter(code => !SYSTEM_ERROR_CODES[code]);

    if (missingCodes.length > 0) {
      throw new Error(`Missing error codes: ${missingCodes.join(', ')}`);
    }

    if (typeof SystemError !== 'function') {
      throw new Error('SystemError is not defined or not a class');
    }

    result.addCheck(canonicalInvariant, true, 'Canonical error classification layer (SystemError) exists', {
      errorClass: 'SystemError',
      hasSchema: true
    });

    result.addCheck(codesInvariant, true, `All ${requiredCodes.length} required error codes defined`, {
      codeCount: Object.keys(SYSTEM_ERROR_CODES).length,
...
function checkInfrastructureModules(result) {
  ...
  try {
    const modules = [
      'error.js (SystemError, SYSTEM_ERROR_CODES)',
