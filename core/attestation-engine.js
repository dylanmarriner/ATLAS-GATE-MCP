...
function getAttestationSecret(workspaceRoot) {
  ...
  let secret = process.env.ATLAS-GATE_ATTESTATION_SECRET;
  if (secret) {
    return secret;
  }

  // Try workspace-specific secret file
  const secretPath = pathModule.join(workspaceRoot, ".atlas-gate", "attestation_secret.json");
  ...
  console.warn(
    "[ATTESTATION] No persistent signing secret found. Using ephemeral secret. " +
      ...
      "Set ATLAS-GATE_ATTESTATION_SECRET or create .atlas-gate/attestation_secret.json"
  );
  return ephemeral;
}
...
function generateAttestationMarkdown(bundle) {
  ...
  lines.push(`\n1. Obtain the signing secret (ATLAS-GATE_ATTESTATION_SECRET)`);
  ...
  lines.push(`\n---`);
  lines.push(`*This attestation was generated by ATLAS-GATE MCP Server and is valid only if signature verification passes.*`);
